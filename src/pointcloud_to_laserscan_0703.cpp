...
public:
    ...
    pointcloud_to_laserscan() : Node("pointcloud_to_laserscan") {
        min_height_ = this->declare_parameter("min_height", 0);
        max_height_ = this->declare_parameter("max_height", 1);
...
private:
    ...
    //pointcloud2 데이터 가공
    std::unique_ptr<sensor_msgs::msg::LaserScan> make_twoD(const sensor_msgs::msg::PointCloud2::SharedPtr c>
        ...
        //할당
        uint32_t ranges_size = std::ceil((scan_msg->angle_max - scan_msg->angle_min) / scan_msg->angle_incr>
